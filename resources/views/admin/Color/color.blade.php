                            ...
                            <tbody>
                                @foreach ($data as $list)
                                    <tr>
                                        <td>{{ $list->id }}</td>
                                        <td>{{ $list->text }}</td>
                                        <td style="background-color:{{$list->value}};
                                        color:white;"></td>
                                        <td>{{ $list->created_at }}</td>
                                        <td>{{ $list->updated_at }}</td>
                                        <td> <button type="button"
                                                onclick="saveData('{{ addslashes($list->id) }}', '{{ addslashes($list->text) }}' , '{{ addslashes($list->value) }}')"
                                                class="btn btn-outline-info px-5 radius-30" data-bs-toggle="modal"
                                                data-bs-target="#exampleModal"> Update</button>
                                           <button onclick="deleteData('{{ addslashes($list->id) }}' , 'colors')"
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form id="formSubmit" action="{{ url('admin/update-color') }}" method="POST"
                        ...
                        <div class="card border-top border-0 border-4 border-info">
                            <div class="card-body">
                                <div class="border p-4 rounded">
                                    ...
                                    <div class="row mb-3">
                                        <label for="enter_text" class="col-sm-3 col-form-label">Enter Color Name</label>
                                        <div class="col-sm-9">
                                      ...
                                      <div class="row mb-3">
                                        <label for="enter_text" class="col-sm-3 col-form-label">Enter Color Value</label>
                                        <div class="col-sm-9">
